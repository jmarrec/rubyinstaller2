...
Server = https://github.com/oneclick/rubyinstaller2-packages/releases/download/ci.ri2
  ...
  if File.read(pacman_conf) =~ /^\[jmarrec\.ri2\]/
    $stderr.puts "pacman repo 'jmarrec.ri2' is already registered"
  else
    $stderr.puts "Register pacman repo 'jmarrec.ri2'"
    File.open(pacman_conf, "a+") do |fd|
      fd.puts
      fd.puts "# Added for RubyInstaller2 arm64 packaging by #{__FILE__}"
      fd.puts <<-EOT
[jmarrec.ri2]
Server = https://github.com/jmarrec/rubyinstaller2-packages/releases/download/ci.ri2
      EOT
    end
    $stderr.puts "Populated #{ pacman_conf }:\n#{ File.read(pacman_conf) }"
  end
  ...
  pacman_key_import(key)
  key = File.read(File.expand_path("../julien-marrec-packaging.asc", __FILE__))
  pacman_key_import(key)

  # Sign the imported keys, so that they're trusted.
  sh "sh -c 'pacman-key --lsign-key BE8BF1C5 --lsign-key AAE32BA7 --lsign-key 93C0327AED032A2E'"
